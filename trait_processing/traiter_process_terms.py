...
##########
# This is a generalization of the trait processing and ordination script reported by Folk et al. 2019 
##########
...
python3 traiter_process_terms.py ./Fagales_2023-01-26/Fagales_2023-01-26.csv ./Manual_trait_extraction_newheader/Fagales_fill-in_controlled_fields.csv ./output/out ./output/codeguide.csv ./output/distancematrix ./all_fields_dropped.csv ./concatenate_terms.csv ./range_terms_quantitative.csv ./range_terms_count.csv 0.95
...
def cleaner(item): # Dataframe elements to lists, alphabetize hyphenated terms, censor, and synonymize
	y = []
	if item != item: # Test for nan
		y = []
		return y
		...
		for x in item: # Iterate over list element
			if x != x: # Test for nan
				break
...
with open(droplist, mode='r', encoding='utf-8-sig') as f: # Encoding to remove u'\ufeff'
    drop = [line.rstrip('\n') for line in f]
#print(len(drop))
traitdataset = traitdataset.drop(drop, axis=1)

#traitdataset = traitdataset.drop(["family","flora_id","flora_name", "text", "raw_traits"], axis = 1) # Axis 1 for columns

print("Number of trait columns after suppression.")
print(len(traitdataset.columns.values))
...
##########
# Smash categorical columns
##########
print("\n\nSmashing categorical columns.")

#print(categorical_field_controlled)
accepted_categorical_fields = list(set(categorical_field_controlled['Accepted term'].tolist())) # Take unique list
#print(terms)
for term in accepted_categorical_fields:
	#print(term)
...
print("Number of trait columns after smashing categorical fields.")
print(len(traitdataset.columns.values))
#print(traitdataset)
#print(traitdataset.columns.to_list())

# Example filled-in field
#print(traitdataset['fruit_part_shape'])


##########
# Smash quantitative and meristic columns
##########

print("\n\nSmashing quantitative and meristic columns.")
accepted_quantitative_fields = list(set(quantitative_field_controlled['Accepted term'].tolist())) # Take unique list
#print(accepted_quantitative_fields)
accepted_meristic_fields = list(set(meristic_field_controlled['Accepted term'].tolist())) # Take unique list
#print(accepted_meristic_fields)

for i,j in zip([accepted_quantitative_fields, accepted_meristic_fields], [quantitative_field_controlled, meristic_field_controlled]):
	for term in i:
		#print(i)
		#print(term)
		#print(j)
		synonyms = j[j['Accepted term'] == term]
		#print(synonyms)
		#print(str(synonyms['Accepted term'].tolist()[0]))
		if len(synonyms['Accepted term'].tolist()) == 1:
			#print(synonyms['Low'].tolist())
			#print(traitdataset[synonyms['Low'].tolist()].values.tolist())
			try:
				traitdataset[term] = traitdataset[synonyms['Low'].tolist()]
			except:
				traitdataset[term] = traitdataset[synonyms['High'].tolist()]
			#print(synonyms['Low extreme'].tolist()[0])
			...
			traitdataset = traitdataset.drop(field_remove_list, axis = 1)
...
print("Number of trait columns after smashing quantitative and meristic fields.")
print(len(traitdataset.columns.values))
#print(traitdataset)
#print(traitdataset.columns.to_list())
...
for i in accepted_categorical_fields:
	traitdataset[i] = traitdataset[i].apply(cleaner)
#print(traitdataset['fruit_part_shape'])


# Format continuous columns
...
missing_number = int((1 - missing) * traitdataset.shape[0] + 1)
#print(missing_number)
traitdataset = traitdataset.loc[:, (traitdataset.astype(bool).sum(axis=0) > missing_number)]
print("Number of trait columns after dropping categorical data due to missing data threshold.")
print(len(traitdataset.columns.values))

traitdataset = traitdataset.replace({'nan':numpy.nan})
traitdataset = traitdataset.dropna(thresh = missing_number, axis = 1)
traitdataset = traitdataset.replace({numpy.nan:'nan'})
...
codesdone = []
# Code the categorical data in alphabetical order sequentially from zero
for w in list_intersection(accepted_categorical_fields, traitdataset.columns):
	#codes = [item for i in codes for item in i] # Flatten list
	#print(w)
	codes = traitdataset[w].to_list()
...
##########
# Select single random member from list for categorical data
##########

print("\n\nSelecting categorical data.")
for i in list_intersection(accepted_categorical_fields, traitdataset.columns):
...
##########
# Prepare for distance method construction
##########
...
for column in traitdataset.select_dtypes([numpy.object]): # Selecting objects needed for empty columns (?)
	...
	try:
		...
		print("".join(["Skipped cleaning of column: ", column]))
		pass
	try:
...
for a in list_intersection(accepted_quantitative_fields, traitdataset.columns):
	traitdataset[a].replace('', numpy.NaN, inplace=True)

categorical_meristic_list = list_intersection(flatten_list([accepted_categorical_fields, accepted_meristic_fields]), traitdataset.columns)

# For discrete data, 0 is a valid datum, so use unoccupied number 
for a in categorical_meristic_list:
	traitdataset[a].replace('', numpy.NaN, inplace=True)
...
traitdataset_matrix = traitdataset[categorical_meristic_list].values
...
continuous_list = list_intersection(accepted_quantitative_fields, traitdataset.columns)

# Standardized euclidean distance
traitdataset_matrix = traitdataset[continuous_list].values
...
combined_dataframe.mask(combined_dataframe == '') # Empty string to NA
...
##########
# Subsample matrix taxa matched to tree, occurrences
##########
...
remove_list = ["Alnus glutinosa"]

print("Number of removed taxa:")
